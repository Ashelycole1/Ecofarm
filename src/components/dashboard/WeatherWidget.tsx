  ...
  return (
    <div className="mh-card p-6 md:p-8 flex flex-col justify-between relative overflow-hidden min-h-[380px] bg-white border border-border-soft group/weather">
      ...
      <div className="grid grid-cols-3 gap-2.5 w-full relative z-10 pt-2 border-t border-border-soft/60">
        <WeatherStat label="Rainfall" value={`${weather.rainfall}mm`} icon={<Droplets size={10} className="text-sienna" />} />
        <WeatherStat label="Wind" value={`${weather.windSpeed}k`} icon={<Wind size={10} className="text-ink-faint" />} />
        <WeatherStat label="Humidity" value={`${weather.humidity}%`} icon={<Thermometer size={10} className="text-forest" />} />
      </div>
    </div>
  )
}
